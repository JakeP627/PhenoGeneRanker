read.settings <- function(input.file){
  ...
  Parameters_File <- read.csv(files$file_name[files$layer_type=="parameters"],
...
read.networks <- function(filesDF){
  # read the gene layer names
  LayerNames <- filesDF$layer_name[filesDF$type%in%c("gene", "cult", "bipartite")]
...
check.files <- function(df){
  
  for (i in 1:nrow(df)){
    assign(df$V1, df$V2)
  }
  
  parameters <- df$V2["parameters"] 
  
  parameters <- list(r,delta,tau,k,lambda,eta, weighted)
  names(parameters) <- c("r", "delta", "tau", "k", "lambda", "eta", "weighted")
  return(parameters)
}
check.parameters <- function(input_file,Nr_Gene_Networks, Nr_Cult_Networks){
  
  if (ncol(input_file) != 2) { stop("Incorrect Parameters Input File")}
  if (nrow(input_file) != 8) { stop("Incorrect Parameters Input File")}
  ...
  tau <- as.numeric(unlist(strsplit(toString(input_file[3,2]),",")))

  #print(paste("Layers Restart Probability: ", input_file[3,2]))
  if (sum(tau)/Nr_Gene_Networks != 1) {stop(sprintf("Incorrect tau, the sum of its component divided by %d must be 1",Nr_Gene_Networks))}
  
  phi <- as.numeric(unlist(strsplit(toString(input_file[4,2]),",")))
  
  #print(paste("Layers Restart Probability: ", input_file[3,2]))
  if (sum(phi)/Nr_Cult_Networks != 1) {stop(sprintf("Incorrect phi, the sum of its component divided by %d must be 1",Nr_Cult_Networks))}  
  
  k <- as.numeric(input_file[5,2] )
  ...
  lambda <- as.numeric(input_file[6,2] )
  #print(paste("Inter Network Jump Probability:", lambda))
  if ((lambda > 1 || lambda < 0)){ stop("Incorrect lambda, it must be between 0 and 1")}
  
  eta <- as.numeric(input_file[7,2] )
  #print(paste("Networks Restart Probabily:", eta))
  if ((eta > 1 || eta < 0)){ stop("Incorrect eta, it must be between 0 and 1")}
  
  weighted <- as.numeric(input_file[8,2] )
  #print(paste("Weighted/Unweighted Network:", weighted))
  if ((weighted != 1 & weighted != 0)){ stop("Incorrect weighted, it must be either 0 or 1")}
  
  
  parameters <- list(r, delta, tau, phi, k, lambda, eta, weighted)
  names(parameters) <- c("r", "delta", "tau", "phi", "k", "lambda", "eta", "weighted")
  return(parameters)
}
...
get.disease.gene.relations <- function(filesDF, proteins){
  gene_cult_file <- filesDF$file_name[filesDF$type=="bipartite"]
  
  Gene_Phenotype_relation <- read.table(gene_cult_file, 
                                        sep="\t", header=TRUE,colClasses = c("character", "character", "numeric"))
  
  ## We remove Gene-phenotype relations that are not in our proteins file. 
  Gene_Phenotype_relation <- Gene_Phenotype_relation[which(Gene_Phenotype_relation$from %in% proteins), ]
...
get.gene.seed.scores <- function(Genes, Number_Layers, tau) {
  ...
  Seeds_Score <- data.frame(Seeds_ID = c(Seed_Genes_Layer_Labeled),
                            Score = c(Seeds_Genes_Scores),stringsAsFactors = FALSE)
  return(Seeds_Score)
}  
create.supraadjacency.matrix <- function(WholeNet, type, N, L, delta, is.weighted.graph){
  # WholeNet <- FullNet
  # type="gene"
  # L <- LG
  ...
  SupraAdjacencyResult <- foreach (i=1:L, .packages=c('igraph', 'Matrix')) %dopar% {
    ...
    SupraAdjacencyMatrixPart[(Position_ini_row:Position_end_row),(Position_ini_col:Position_end_col)] <- (1-delta)*(Adjacency_Layer)
    ...
    for (j in 1:L){
      ...
      if (j != i){
        SupraAdjacencyMatrixPart[(Position_ini_row:Position_end_row),(Position_ini_col:Position_end_col)] <- (delta/(L_mdfd))*Idem_Matrix
      }
    }
...
create.suprabibartite.matrix <- function(Bipartite_matrix, N, M, LG, LC){
...
add.rap.mapping <- function(final_rank, baseDir, mapFile){
  rapMsu <-read.table(paste0(baseDir,"datasets/id_map/", mapFile), sep="\t", 
                      header=TRUE, stringsAsFactors = FALSE) 
  
  # omit the NA rap_id rows
  rapMsu <- rapMsu[-which(is.na(rapMsu$rap_id)),] 
  
  # omit the duplicated msu_id rows, this could be changed
  rapMsu <- rapMsu[-which(duplicated(rapMsu$msu_id)),]
  
  
  final_rank <- base::merge(final_rank, 
                            rapMsu, 
                            by.x="Gene",
                            by.y="msu_id",
                            all.x = TRUE)  
  # rename the rap_id column to RAP_ID
  colnames(final_rank)[ncol(final_rank)] <- "RAP_ID"
  # make the RAP_ID second column
  final_rank <- final_rank[,c(1,ncol(final_rank),2:(ncol(final_rank)-1))]
  # order by Rank
  final_rank <- final_rank[order(final_rank$Rank),] 
  final_rank
}
...
get.ResultID <- function(outputDir, WM_ID, Layers, GeneSeeds, CultSeeds, eta){
  Seeds <- c(GeneSeeds, CultSeeds)
  ResultDF <-  read.table(paste0(outputDir, "/ResultDF.txt"), header=TRUE, sep="\t", stringsAsFactors = FALSE) 
  
  newRes <- data.frame(ID=ifelse(length(ResultDF$ID)==0, 1, max(ResultDF$ID)+1),
                       Seeds=paste0(paste0(Seeds, collapse = ","), ",eta=",toString(eta)), 
                       WM=WM_ID, Layers=Layers, Date=as.character(Sys.time()))
  ResultDF <- rbind(ResultDF, newRes)
  res <- try(write.table(ResultDF, paste0(outputDir, "/ResultDF.txt"), sep="\t", 
              col.names = TRUE, row.names = FALSE, quote=FALSE))
  if(!is.null(res)) stop("Eror: Could not get Result ID!")
  return(max(ResultDF$ID))
}
create.WalkMatrix <- function(inputFileName, numCores){
  ...
  t1 <- Sys.time()
  SupraBipartiteMatrix <- create.suprabibartite.matrix(BipartiteMatrix, N, M, LG, LC)
...
check.network.weight.range <- function(df, layer_name){
  if(!all(range(df$weight)==network_range)) 
    stop(paste0("Range of the layer '", layer_name,"' is outside of the expected range: [", toString(range(df$weight)), "]!"))
}
Random_Walk_Restarts_Batch <- function(Walk_Matrix, GeneSeedsList, CultSeedsList, N, LG, LC, eta, tau, phi, r, funcs, no.cores=15){
  # t <- Sys.time()
  cl <- makeCluster(no.cores)
  registerDoParallel(cl)
